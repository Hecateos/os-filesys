...
int rm_dir(struct Entry *pentry)
{
	int cluster_addr, ret, offset, i;
	unsigned char buf[DIR_ENTRY_SIZE];
	struct Entry *p;
	unsigned char c;
	unsigned short seed,next;
	cluster_addr = (pentry->FirstCluster -2 )*CLUSTER_SIZE + DATA_OFFSET;
	if((ret= lseek(fd,cluster_addr,SEEK_SET))<0)
		perror("lseek cluster_addr failed");
	offset = cluster_addr;
	while(offset < cluster_addr + CLUSTER_SIZE)
	{
		...
		if(p->short_name[0] == 0xe5 || p->short_name[0] == 0x00)
			continue;
		/*clear FAT*/
		seed = p->FirstCluster;
		while((next = GetFatCluster(seed))!=0xffff)
...
int fd_rm_r(char *dirname)
{
	struct Entry *pentry;
	int ret, offset, cluster_addr, ifEmpty = 1;
	char input[5];
	unsigned char c;
	unsigned short seed,next;
	unsigned char buf[DIR_ENTRY_SIZE];
...
{
	struct Entry *pentry;
	int ret, offset, cluster_addr, temp, ifEmpty = 1;
	char input[5];
	unsigned char c;
	unsigned short seed,next;
	unsigned char buf[DIR_ENTRY_SIZE];
	...
	{
		printf("确定删除%s目录及其包含的所有子目录和文件吗？y/n\t", dirname);
		scanf("%s", input);
		...
		{ 
			...
			while(offset < DATA_OFFSET)
			{
				...
				{       
					...
					c[22] = (p->tm_min << 5) | (p->tm_sec / 2);
					c[23] = (p->tm_hour+8 << 3) | (p->tm_min >> 3);
					c[24] = ((p->tm_mon+1) << 5) | p->tm_mday;
					c[25] = ((p->tm_year-80) << 1) | ((p->tm_mon+1) >> 3);
...
void clear(int clusterno)
{
	int cluster_addr, ret, offset;
	unsigned char c;
	c = 0xe5;
	cluster_addr = (clusterno -2 )*CLUSTER_SIZE + DATA_OFFSET;
	if((ret= lseek(fd,cluster_addr,SEEK_SET))<0)
		perror("lseek cluster_addr failed");
	offset = cluster_addr;
	while(offset < cluster_addr + CLUSTER_SIZE)
...
int pathSwitch(char *pathname, char *name)//duo ji mu lu
{
	int num = 0, i, j, k = 0, ret;
	struct Entry *pentry;
	if(pathname[0] == '/')
	{
		k = 1;
...
void do_usage()
{
	...
	printf("rm <dir>\t\tdelete a dir with hint\n\trm -r <dir>\t\tdelete a dir\n\texit\t\t\texit this system\n");
}
...
int main()
{
	...
	while (1)
	{
		...
		else if(strcmp(input, "cd") == 0)
		{
			...
			if((num = pathSwitch(pathname, name)) == -1)
			{
				continue;
			}
			fd_cd(name);
