...
int WriteFat()
{
	...
	if(write(fd,fatbuf,SECTOR_SIZE*SECTTORS_PER_FAT)<0)
	{
		perror("read failed");
		return -1;
	}
...
int ReadFat()
{
	...
	if(read(fd,fatbuf,SECTOR_SIZE*SECTTORS_PER_FAT)<0)
	{
		perror("read failed");
		return -1;
	}
...
void fd_rnfile(char *oldname,char *newname){
	struct Entry *pentry;
	int ret,i;
	unsigned char c[12];
	unsigned short seed,next;
	...
	ret = ScanEntry(oldname,pentry,0);
	if(ret<0)
	{
		printf("no such file\n");
		free(pentry);
	...
	for(i=0;i<=strlen(newname);i++)
		c[i]=toupper(newname[i]);
	for(;i<=10;i++)
		c[i]=' ';
	if(lseek(fd,ret-0x20,SEEK_SET)<0)
		perror("lseek rnfile failed");
	if(write(fd,&c,11)<0)
		perror("write failed"); 
	free(pentry);
}
...
int main()
{
	...
	while (1)
	{
		...
		else if(strcmp(input, "rnfile") == 0)
		{
			scanf("%s", oldname);
			scanf("%s", newname);
			fd_rnfile(oldname,newname);
		}
		else
		 	do_usage();
	}	
